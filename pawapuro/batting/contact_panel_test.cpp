...
int main(int argc,char** argv){try {
    ...
    const double decision_start=p.ball_passage.enter_s-potential.normal_end_ms/1000.;
    const double decision_end=p.ball_passage.exit_s-potential.normal_start_ms/1000.;
    const double duration=decision_end-decision_start;
    require(std::isfinite(duration)&&duration>0,"finite positive decision duration");
    ...
    require(std::abs(empty.timeline.decision_start-1./6)<1e-12&&std::abs(1-empty.timeline.decision_end-1./6)<1e-12
        &&std::abs(empty.timeline.decision_end-empty.timeline.decision_start-2./3)<1e-12,"decision window dominance and equal margins");
    std::cout<<std::setprecision(17)<<"TIMELINE ball_enter="<<p.ball_passage.enter_s<<" ball_exit="<<p.ball_passage.exit_s
        <<" decision_start="<<decision_start<<" decision_end="<<decision_end<<" duration="<<duration<<" start="<<display_start<<" end="<<display_end
        <<" yellow_fraction="<<empty.timeline.decision_end-empty.timeline.decision_start<<'\n';
    require(empty.timeline.decision_start==timeline_position(decision_start)&&empty.timeline.decision_end==timeline_position(decision_end)
        &&empty.timeline.decision_start>0&&empty.timeline.decision_start<empty.timeline.decision_end&&empty.timeline.decision_end<1,"visible authoritative yellow interval");
    std::array<double,5> contact_positions{},commit_times{},resolved_times{};unsigned contact_index=0;
    for(const BattingReviewFixture fixture:{BattingReviewFixture{436,0,0},{444,0,0},{441,0,0},{448,0,0},{455,0,0},{80,0,0},{480,0,0},{448,.6f,1}}){
        ...
        for(const std::uint64_t cadence:{4'166'667ULL,33'333'333ULL,8'333'333ULL,250'000'000ULL}){
            ...
            require(info.timeline.marker==(contact?TimingMarker::Contact:TimingMarker::SwingWithoutContact),"timeline marker kind");
            const double event=double(p.committed()->consumed_tick)/pitch_hz;
            require(p.committed()->consumed_tick==fixture.commit_tick,"accepted fixture commit changed");
            if(contact_index<5)require(contact,"centered regression lost Contact");
            if(fixture.ex!=0)require(!contact&&info.timeline.marker_position==contact_positions[3],"same commit must share position despite spatial Miss");
            if(!control){
                std::cout<<"TIMELINE tick="<<fixture.commit_tick<<" event="<<event<<" position="<<info.timeline.marker_position<<" ms="<<*info.offset_ms<<'\n';
                if(contact_index<5){commit_times[contact_index]=event;resolved_times[contact_index]=p.latest()->response->contact_time_s;}
            }
            ...
            require(info.timeline.decision_start==empty.timeline.decision_start&&info.timeline.decision_end==empty.timeline.decision_end,"attempt moved interval");
            ...
            if(fixture.commit_tick==80){
                require(info.timeline.marker_position==0&&*info.offset_ms<-1000,"early commit clamp preserves full offset");
                char expected[32];std::snprintf(expected,sizeof(expected),"%+.1f ms",p.timing()->offset_ms);
                require(std::strcmp(format_batting_info(info)[0].data(),expected)==0,"clamped commit changed ms text");
            }
            if(fixture.commit_tick==480)require(info.timeline.marker_position==1,"late commit clamp");
        }
        if(contact_index<5)contact_positions[contact_index++]=control->timeline.marker_position;
    }
    require(resolved_times[0]==resolved_times[1]&&commit_times[0]<commit_times[1]&&contact_positions[0]<contact_positions[1],"436/444 decisions collapsed with contact time");
    require(std::abs((contact_positions[1]-contact_positions[0])-(commit_times[1]-commit_times[0])/(display_end-display_start))<1e-12,"decision mapping is not linear");
    require(contact_positions[2]<contact_positions[3]&&contact_positions[3]<contact_positions[4],"441/448/455 vertical ordering");
    ...
    for(unsigned height:{1080u,1620u}){
        ...
        const auto render=[&]{
            ...
            require(drawn[band].position.y==1-(100+144*float(before.timeline.decision_start))*panel.y_scale
                &&drawn[band+2].position.y==1-(100+144*float(before.timeline.decision_end))*panel.y_scale
        ...
        for(const BattingReviewFixture fixture:{BattingReviewFixture{448,0,0},{448,0,.5f},{448,0,.75f},{448,0,.9f},{448,0,-.5f},{448,0,-.75f},{448,0,-.25f},{448,0,.25f},{448,.6f,1},{80,0,0},{456,0,0},{0,0,0}}){
            ...
            while(p.phase==PreviewPhase::Playing){
                ...
                if(p.latest()){
                    ...
                    require(info.timeline.marker==(dispatched?TimingMarker::Contact:TimingMarker::SwingWithoutContact)
                        &&info.timeline.marker_position==timeline_position(double(p.committed()->consumed_tick)/pitch_hz),"planned contact presented as dispatched or wrong commit time");
