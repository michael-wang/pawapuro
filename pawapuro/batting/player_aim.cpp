...
namespace pawapuro {
PlayerAim::PlayerAim(const BattingStaging& s):tuning(s.player_aim),half_width(s.strike_zone_width_m/2),
    ...
    visual_z(plane_z-0.002f) // Fixed camera is on -Z; rendering only, never aim truth.
{
    if (!std::isfinite(half_width)||half_width<=0||!std::isfinite(bottom)||!std::isfinite(top)||top<=bottom||
        !std::isfinite(tuning.normal_radius_x_m)||tuning.normal_radius_x_m<=0||tuning.normal_radius_x_m>=2*half_width||
        !std::isfinite(tuning.normal_radius_y_m)||tuning.normal_radius_y_m<=0||tuning.normal_radius_y_m>=top-bottom||
...
AimDiagnostic PlayerAim::diagnostic(DirectX::XMFLOAT3 p) const {
    const float dx=p.x-center_m.x,dy=p.y-center_m.y;
    const float ex=dx/tuning.normal_radius_x_m,ey=dy/tuning.normal_radius_y_m;
    return {dx,dy,ex,ey,ex*ex+ey*ey};
}
void PlayerAim::append_triangles(std::vector<engine::Vertex>& v) const {
    ...
    const float rx=tuning.normal_radius_x_m,ry=tuning.normal_radius_y_m;
