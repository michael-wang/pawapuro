...
BatContact result(const BatterMotion& batter,const ContactSample& s,BatContactEnvelope e,double phase_offset) {
    ...
    const auto a=batter.sample_barrel(s.preview_time_s+phase_offset-epsilon,scratch),b=batter.sample_barrel(s.preview_time_s+phase_offset+epsilon,scratch);
...
{
    if((substeps!=32 && substeps!=64 && substeps!=128) || !std::isfinite(start) || !std::isfinite(end) || !std::isfinite(phase_offset)
    ...
    auto previous=sample_contact(initial,release,batter,start,scratch,phase_offset);
    if(previous.approach.distance_m<=radius) return result(batter,previous,envelope,phase_offset);
    for(unsigned i=1;i<=substeps;++i) {
        auto next=sample_contact(initial,release,batter,start+(end-start)*i/substeps,scratch,phase_offset);
        if(next.approach.distance_m<=radius) {
            ...
            for(unsigned step=0;step<24;++step) {
                const double middle=(low+high)*.5;const auto candidate=sample_contact(initial,release,batter,middle,scratch,phase_offset);
                if(candidate.approach.distance_m<=radius) {high=middle;next=candidate;} else low=middle;
            }
            return result(batter,next,envelope,phase_offset);
        }
        previous=next;
    }
    return std::nullopt;
