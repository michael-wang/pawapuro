...
std::string vertical_contact_study(const ManualSwingPreview& p) {
 ...
 out<<"# Current production diagnostic only; no future angle/family contract\n"
    ...
    <<"# rx="<<region.normal_radius_x_m<<" ry="<<region.normal_radius_y_m<<" rb="<<s.gameplay_ball.radius_m
    <<" max_spray_deg="<<tuning.max_spray_degrees<<" full_spray_offset_ms="<<tuning.full_spray_offset_ms<<'\n'
    <<"# Authorization evaluated in pitch-relative XY coordinates to represent +/-(ry+rb) axis tangency exactly at q=1\n"
 ...
 for(float ey:{-1.f,-.95f,-.90f,-.75f,-.50f,-.10f,0.f,.10f,.50f,.75f,.90f,.95f,1.f}) {
  const DirectX::XMFLOAT2 aim{0,-ey*(region.normal_radius_y_m+s.gameplay_ball.radius_m)};
  const auto auth=authorize_normal_hit(aim,{0,0},region,s.gameplay_ball);
...
int main(int argc,char** argv){try{
 ...
 float prior_q=100;for(int contact:{0,75,120}){
  ...
  require(other.gameplay_ball.radius_m==.085f,"Contact scaled gameplay ball");
  ...
  const auto normalized=authorize_normal_hit({0,0},{(r.normal_radius_x_m+s.gameplay_ball.radius_m)*.5f,(r.normal_radius_y_m+s.gameplay_ball.radius_m)*.5f},r,s.gameplay_ball);
  ...
  PlayerAim visual(other);const auto vc=visual.center();const auto diagnostic=visual.diagnostic({vc.x+(r.normal_radius_x_m+s.gameplay_ball.radius_m)*.5f,vc.y+(r.normal_radius_y_m+s.gameplay_ball.radius_m)*.5f,0});
  require(close_enough(diagnostic.q,.5),"visual aim uses a different authorization region");
  const auto absolute=authorize_normal_hit({0,0},{.10f,.04f},r,s.gameplay_ball);require(absolute.q<prior_q,"Contact did not reduce same-metre q");prior_q=absolute.q;
 }
 ...
 const auto run=[&](std::uint64_t c,float ex=0,float ey=0){p.reset();p.start();require(p.record_command(c,{center.x-ex*(region.normal_radius_x_m+s.gameplay_ball.radius_m),center.y-ey*(region.normal_radius_y_m+s.gameplay_ball.radius_m)}),"fixture rejected");finish(p);return p.attempts.back();};
 ...
 for(float ey:{-.9f,.9f}) {
  const auto skin=run(448,0,ey);
  require(skin.authorization.authorized&&std::abs(skin.authorization.error.y)>region.normal_radius_y_m&&close_enough(skin.authorization.normalized_error.y,ey)&&skin.gameplay==GameplayResult::Contact&&skin.response->launch_velocity_mps.z<0,"authorized skin contact cannot reach extreme topology");
  std::cout<<"SKIN commit=448 dy="<<skin.authorization.error.y<<" ey="<<skin.authorization.normalized_error.y<<" authorized=1 longitudinal="<<skin.response->longitudinal_angle_deg<<" ground_s="<<p.flight->ground_s<<'\n';
 }
 auto a=run(448);
