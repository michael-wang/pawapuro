...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        const pawapuro::BatterCard batter_card(staging.batter_profile,static_cast<unsigned>(width),static_cast<unsigned>(height));
        const auto& profile=staging.batter_profile;
        std::fprintf(stderr,"BatterProfile name=%s contact=%d(%c) power=%d(%c) trajectory=%d\n",
        ...
        dynamic_characters.reserve(motion.triangles.size()+batter.triangles.size()+pawapuro::PlayerAim::vertex_count+pawapuro::ball_readability_vertex_count+pawapuro::arrival_cue_vertex_count+result_panel.vertex_count+batter_card.vertex_count());
