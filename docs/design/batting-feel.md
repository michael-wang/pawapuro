...
實際 subset／啟動入口見 [pitcher README](../../pawapuro/batting/pitcher/README.md#s1-static-bind-pose-runtime)，依賴與本輪驗證見 [environment](../development/environment.md#s1-static-pitcher-glb-runtime-import2026-09-16)。Runtime animation／skinning、rubber-arm 動態造型、release／ball attachment、dynamic occlusion 與 early-flight readability 尚未驗證，M1 未完成。以上是 S1 的接受邊界。S2 與 S3 現況見下節。[Character Motion Rules v0.1](character-motion.md) 已獲 Michael＋Julia design acceptance；六條 rules 不因 runtime 實作改寫。


## S2：Runtime Pitcher Animation Playback／CPU Skinning（2026-09-16）

S2 已通過 Michael＋Julia 技術與 human review：正式 S0.2C motion 能正確在 runtime 播放。Character Style debt 由 [Character Motion Rules](character-motion.md) 維護；不代表 style、release integration 或 M1 通過。v1A 的 1.25× planar footprint 已接受並原樣升為正式資產；Foot Shape v1B geometry 已 human accepted／promoted，front-foot geometry 朝向已診斷正確，v1C shoe silhouette 已 human rejected、正式 v1B 保留；Rubber Arm v1A 暫緩 promotion；Throwing Arm Whip Timing v1A 依 Rule 3／6 已 human accepted／promoted，保留 body timing，以 post-release 右臂壓縮形成速度對比；material／highlight、articulation／sole／cleats、rubber arm 仍待獨立工作，沿用此節 runtime code／契約。
...
- **球的邊界**：app 不再把 S1 bind pitcher 烘進場景，只畫 dynamic pitcher。Reference ball／prediction 是獨立靜態 fixture，Space 只播放動作；不 attachment、不隱藏／freeze handoff、不觸發 marker event。ReferencePitch simulation／prediction 與其 regression 保留，這是 S2 交付時的邊界；目前 app ownership transfer 由下節 S3 取代。
...
- **Gameplay completion 與 presentation completion 分開**：正式球在 delivery 479／pitch 95 到 evaluation plane，`PitchPhase::Complete` 與 `PitchArrival` 立即可用。球停止積分、保留 raw Complete state，不吸附 interpolated plane sample；動畫繼續原 follow-through 到 tick 816。兩個 child 都 Complete 才進入 delivery Complete。Recovering 只是「pitch 已完成、動畫仍 active」的顯示 sub-state，不加另一個 state framework；preview replay 必須等整個 delivery 完成。
...
正式 Whip Timing v1A、v1B 鞋、support／contact timing 與所有資產不改。Rule 5 的 semantic attachment 在此實作，六條 Character Motion Rules 原文不改。操作與證據見 pitcher README；驗證由 environment 維護。Cyan diagnostic、dynamic occlusion、material／style 可保留為後續 presentation debt，不阻擋 batter 工作；正式遊戲品質與 M1 未完成。[Batter Motion Brief](batter-motion.md) 已獲 Michael＋Julia 接受；Batter Authoring S0 左打 full-swing baseline 亦已獲 Michael＋Julia 接受。本次 [Batter Runtime S1](../../pawapuro/batting/batter/README.md#batter-runtime-s1--static-bind-import-candidate) 將正式 GLB 的兩 named mesh primitives 以共享 hierarchy／skin／clip 載入，bind/rest geometry 進既有 immutable scene buffer，移除 procedural batter；Engine 不知道 bat gameplay，renderer／HLSL 未改。S1 static transport／shared hierarchy、placement／scale／handedness、attachment／grounding、composition／sightline 已獲 Michael＋Julia human acceptance。S2 現況見下節；collision／contact presentation 未開始。
...
Bat Contact S0 在 accepted S2 上新增 read-only geometry study：球用 authoritative initial state 的 constant-acceleration query，棒用 scratch pose 取既有 barrel→tip semantics，在 contact_area ±8 ticks 固定 refinement。只量 point-to-segment centerline separation，不使用 frozen Complete 球、visual ball radius 或 bat mesh thickness 決定接觸。App sequencing／simulation／renderer 不變；結果、收斂與證據由 [batter README](../../pawapuro/batting/batter/README.md#bat-contact-s0--continuous-closest-approach-probe) 擁有。S0 probe 已獲 Michael＋Julia acceptance；S1 envelope／event 見下段，response／sweet spot 與正式 swing input 未開始。


Bat Contact S1 已獲 Michael＋Julia acceptance。`staging.toml [bat_contact]` 的 37 mm ball／33 mm bat 是第一可調 gameplay envelope，Reality is a consultant，與 enlarged visual marker／rendered thickness 分離。Native 在目前 bounded contact window 的每 tick interval 找 sphere 對 barrel→tip capsule 的 earliest entry，BattingPreview 每次播放僅保存／log 一次 event，球路／ownership／動畫及完成語意不變。Endpoint hemisphere 合法；contact_area 不 trigger、不 snap。現行 overlap 時窗小於一個240 Hz tick，須 sub-tick detection；具體精度／範圍與數值由 [batter README](../../pawapuro/batting/batter/README.md#bat-contact-s1--first-continuous-contact-event-candidate) 維護。不定 sweet spot／response、hit quality 或正式 player input。


Bat Contact S2 是read-only phase-offset geometry study：只在non-mutating sampler令bat clip time=preview time+offset，positive代表phase超前，並非player input delay。正式runtime維持offset0。S1已接受的37／33 mm envelope、球路、swing、continuous entry均不改；41點coarse後因Contact碰到邊界擴到±60 ms，僅做一次local refinement，數值／未知邊界由[batter README](../../pawapuro/batting/batter/README.md#bat-contact-s2--timing-to-contact-geometry-map2026-09-17)擁有。normal.y>0且interior的區域只是geometry interpretation，不定sweet spot、Perfect Hit、timing window或response。Study已獲Michael＋Julia acceptance。
